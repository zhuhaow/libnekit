...
inline bool is_chars(const char *s, const char *e, std::uint8_t mask) {
  while (s != e)
    if (!is_char(*s++, mask)) return false;
  return true;
}
...
inline bool is_uint(const char *&s, const char *e, uint32_t max) {
  if (s == e || !is_num(*s)) return false;
  const char *t = s;
  uint32_t val = *t++ - '0';
  if (val)
    while (t != e && is_num(*t)) val = val * 10 + (*t++ - '0');
...
inline bool is_scheme(const char *s, const char *e) {
  if (!s || !e || s == e || !is_alpha(*s)) return false;
  char c;
  while (++s != e)
    if (!is_alnum(c = *s) && c != '+' && c != '-' && c != '.') return false;
  return true;
}
...
inline bool is_ipv4(const char *s, const char *e) {
  size_t l = e - s;
  if (l < 7 || l > 254) return false;
  for (const char *p = s; p != e; ++p)
    if (*p != '.' && !is_num(*p)) return false;
  return true;
}
...
bool is_ipv6(const char *s, const char *e) {
  size_t l = e - s;
  if (l < 2 || l > 254) return false;
  for (const char *p = s; p != e; ++p)
    if (*p != ':' && *p != '.' && !is_hexdigit(*p)) return false;
  return true;
}
...
bool is_valid_ipv6(const char *s, const char *e) {
  ...
  while (p != e) {
    if (*p == '.') {
      ...
      if ((++ndigits > 4) || !is_hexdigit(*p++)) return false;
    }
  }
  if (ndigits > 0)
    ++nfields;
...
std::string normalize_IPv6(const char *s, const char *e) {
  ...
  if ((e - s) == 2 && s[0] == ':' && s[1] == ':') return std::string(s, e - s);
  ...
  if (null_pos == 0) {
    ...
    for (i = 1; i < null_pos; ++i) str << ':' << fields[i];
    if (i < nfields) str << ':';
    i += null_len;
    if (i == 8 && null_len != 0) str << ':';
  }
  for (; i < nfields; ++i) str << ':' << fields[i];
  if (ipv4_b) str << ':' << std::string(ipv4_b, ipv4_e - ipv4_b);
...
void Url::assign(const Url &url) {
  ...
  if (!m_parse || m_built) m_url = url.m_url;
}
...
void Url::assign(Url &&url) {
  ...
  if (!m_parse || m_built) m_url = std::move(url.m_url);
}

Url &Url::scheme(const std::string &s) {
  if (!is_scheme(s)) throw Url::parse_error("Invalid scheme '" + s + "'");
  lazy_parse();
  std::string o(s);
  to_lower(o);
  if (o != m_scheme) {
...
Url &Url::port(const std::string &p) {
  if (!is_port(p)) throw Url::parse_error("Invalid port '" + p + "'");
  lazy_parse();
  std::string o(p);
...
void Url::parse_url() const {
  ...
  std::int8_t ip_v = -1;
  const char *scheme_b, *scheme_e, *user_b, *user_e, *host_b, *host_e, *port_b,
      *port_e, *path_b, *path_e, *query_b, *query_e, *fragment_b, *fragment_e;
  ...
  if (p == e) {
    ...
    if (*p == ':') {
      if (!is_scheme(b, p))
        throw Url::parse_error("Scheme in '" + std::string(s, e - s) +
                               "' is invalid");
      scheme_b = b;
      scheme_e = p;
  ...
  if (scheme_b) _scheme = normalize_scheme(scheme_b, scheme_e);
  if (user_b) _user = decode(user_b, user_e);
  if (host_b) {
  ...
  if (port_b) _port = std::string(port_b, port_e - port_b);
  if (path_b) _path = normalize_path(decode(path_b, path_e));
  if (query_b) {
    ...
    while (p != query_e) {
      ...
      if (p != query_e) {
        if (*p == '=') {
          ...
          if (!is_chars(b, p, 0x3F))
            throw Url::parse_error("Query value '" + std::string(b, p) +
                                   "' in '" + std::string(s, e - s) +
                                   "' is invalid");
          val = decode_plus(b, p);
        }
  ...
  if (fragment_b) _fragment = decode(fragment_b, fragment_e);
...
void Url::build_url() const {
  ...
  if (!m_scheme.empty()) url << m_scheme << ":";
  if (!m_host.empty()) {
    url << "//";
    if (!m_user.empty()) url << encode(m_user, 0x05) << '@';
  ...
  if (!m_query.empty()) {
    ...
    if (!it->val().empty()) url << "=" << encode_query_val(it->val(), 0x1F);
    while (++it != end) {
      if (it->key().empty()) throw Url::build_error("A query entry has no key");
      url << "&" << encode_query_key(it->key(), 0x1F);
      if (!it->val().empty()) url << "=" << encode_query_val(it->val(), 0x1F);
    }
  }
  if (!m_fragment.empty()) url << "#" << encode(m_fragment, 0x1F);
  m_built = false;
  m_url = url.str();
}
...
std::ostream &Url::output(std::ostream &o) const {
  ...
  if (!m_query.empty()) {
    std::stringstream str;
    str << " query(";
    for (const auto &q : m_query) str << q;
