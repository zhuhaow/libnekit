...
#define nsel_CPP11_OR_GREATER \
  (__cplusplus >= 201103L || type_MSVC_LANG >= 201103L)
#define nsel_CPP14_OR_GREATER \
  (__cplusplus >= 201402L || type_MSVC_LANG >= 201703L)
#define nsel_CPP17_OR_GREATER \
  (__cplusplus >= 201703L || type_MSVC_LANG >= 201703L)
#define nsel_CPP20_OR_GREATER \
  (__cplusplus >= 202000L || type_MSVC_LANG >= 202000L)
...
#define nsel_REQUIRES_T(...)                        \
  typename = typename std::enable_if<(__VA_ARGS__), \
                                     nonstd::expected_detail::enabler>::type
...
// - C26409: Avoid calling new and delete explicitly, use std::make_unique<T>
// instead (r.11)
...
nsel_DISABLE_MSVC_WARNINGS(26409)
  ...
  /// // x.x.3 Unexpected object type; unexpected_type; C++17 and later can also
...
#endif  // nsel_P0323R
  {
   public:
    ...
    template <typename E2,
              nsel_REQUIRES_T(std::is_constructible<E, E2 &&>::value)>
  ...
  template <typename E>
  void swap(unexpected_type<E> & x,
            unexpected_type<E> & y) noexcept(noexcept(x.swap(y))) {
    x.swap(y);
  }
  ...
  /// in-place tag: construct a value in-place (C++17 std::in_place in
  /// <utility>)
...
#endif  // nsel_P0323R
  {
   public:
    ...
    nsel_constexpr14 explicit expected(
        ...
        nsel_REQUIRES(!std::is_convertible<G &&, E>::value /*=> explicit */))
        : has_value_(false) {
    ...
    nsel_constexpr14 /*non-explicit*/ expected(
        nonstd::unexpected_type<G> &&error,
        nsel_REQUIRES(std::is_convertible<G &&, E>::value /*=> non-explicit */))
        : has_value_(false) {
      contained.construct_error(std::move(error.value()));
    }
    ...
    template <typename U,
              nsel_REQUIRES_T(std::is_constructible<T, U>::value
    ...
    bool has_exception() const {
      using ContainedEx = typename std::remove_reference<decltype(
  ...
  class expected<void, E> {
   public:
    ...
    template <typename U, typename... Args,
              nsel_REQUIRES_T(std::is_constructible<U, std::initializer_list<U>,
                                                    Args &&...>::value)>
    nsel_constexpr14 explicit expected(unexpect_t, std::initializer_list<U> il,
                                       Args &&... args)
        : has_value_(false) {
